...
type CheckoutSummaryProps = {
  ...
  onApprovePayPalOrder: (data: { orderID: string }) => Promise<void>
  onViewOrder: () => void
}
      ...
      <div>
        ...
        <div className='space-y-2'>
          ...
          {!isPaid && paymentMethod === 'Cash On Delivery' && (
            <Button className='w-full rounded-full' onClick={onViewOrder}>
              View Order
            </Button>
          )}
        </div>
...
export default function OrderDetailsForm({
  ...
  const handleCreatePayPalOrder = async () => {
    ...
    if (!res.success) {
      ...
      throw new Error(res.message) // ← throw instead of return undefined
    }
  ...
  const summaryProps: CheckoutSummaryProps = {
    ...
    onApprovePayPalOrder: handleApprovePayPalOrder,
    onViewOrder: () => router.push(`/account/orders/${order._id.toString()}`),
  }
